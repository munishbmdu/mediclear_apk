    ...
    if(response.statusCode==200){
      ...
      var data=dataa['Userdata'];
       email.text=data['email']==null?"":data['email'].toString();
            ...
            phone.text=data['mobile_no']==null?"":data['mobile_no'].toString();
 ...
 getProfileData();
                             ...
                             return CircleAvatar(
                                radius: 60,
                                 // backgroundImage: img!= "customer profile not updated"? NetworkImage(img,):null ,
                                   child:ClipOval(
                                   child:imageFile2==null? img== "customer profile not updated"?Icon(Icons.person):Image.network(img,fit: BoxFit.cover,width: 120,height: 130,):Image.file(File(imageFile2!.path,),fit: BoxFit.cover,height: 130,width: 120,))
                              ...
                              content: FutureBuilder(
                                ...
                                if(!snapshot.hasData){
                                  return Container(
                                    height: 50,
                                    width: 50,
                                    child: Center(child: Text("Wait a moment")),
                                  );
                                ...
                                }else if(snapshot.hasData)
                                {
                                  return ListView.builder(
                                    ...
                                    itemBuilder: (context,index){
                                     return ListTile(
                                        onTap: (){
                                          state.text= snapshot.data[index]["name"].toString();
                      ...
                      child: TextField(
                        ...
                        //readOnly: true,
                          ...
                          decoration: InputDecoration(
                            ...
                            return AlertDialog(
                              content: FutureBuilder(
                                future: getState("city", "state", id), builder:(context,snapshot){
                                if(!snapshot.hasData){
                                  return Container(
                                    height: 50,
                                    width: 50,
                                    child: Center(child: Text("Wait a moment or enter valid state")),
                                  );
                                ...
                                }else if(snapshot.hasData)
                                {
                                  return ListView.builder(
                                    ...
                                    itemBuilder: (context,index){
                                     return ListTile(
                                        onTap: (){
                                          city.text= snapshot.data[index]["name"].toString();
                                             Navigator.pop(context);
  ...
  _cropImage(File imgFile) async {
   ...
   Future<void> postProfileData(var email,state,city,pincode, context) async {
    ...
    var uri = Uri.parse("${ApiDomain().url}customer-profile"); // Replace with your server's endpoint
    var request = http.MultipartRequest("POST", uri)
       ..files.add(await http.MultipartFile.fromPath('customer_profile_image',imageFile2!.path,));
  ...
  Future getState(String state_city,String country_state,int id)async{
    final response=await http.get(Uri.parse("https://demo.codingbirdsonline.com/country-state-city-dropdown-api/index.php?endpoint=get${state_city}&${country_state}_id=${id}"));
    if(response.statusCode==200){
